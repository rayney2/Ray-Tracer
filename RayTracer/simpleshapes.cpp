...
public:
    ...
    bool intersect(const Vec3f& orig, const Vec3f& dir, float& t) const
    {
        ...
        Vec3f rayInv = Vec3f(1 / dir.x, 1 / dir.y, 1 / dir.x);
        int sign[3];
        sign[0] = (rayInv.x < 0);
        sign[1] = (rayInv.y < 0);
        sign[2] = (rayInv.z < 0);
...
int main(int argc, char **argv)
{
    ...
    for (uint32_t i = 0; i < numSpheres; ++i) {
        float randNum = (0.5 - dis(gen) * 10);
        AABBox *box = new AABBox(Vec3f(-randNum, -randNum, -randNum), Vec3f(randNum, randNum, randNum));
       // AABBox *box1 = new AABBox(Vec3f(-1), Vec3f(1));
        objects.push_back(std::unique_ptr<Object>(box));
    }
