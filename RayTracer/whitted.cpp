...
float dotProduct2(const Vec2f& a, const Vec2f& b)
{
...
public:
    Cylinder(const Vec3f& start, const Vec3f& end, const float& r) {
        if ((end.x - start.x) > (end.y - start.y) && (end.x - start.x) > (end.z - start.z)) axis = 1;
        if ((end.y - start.y) > (end.z - start.z) && (end.y - start.y) > (end.x - start.x)) axis = 2;
        if ((end.z - start.z) > (end.y - start.y) && (end.z - start.z) > (end.x - start.x)) axis = 3;
        
        if (axis == 1) center = (end.x / 2, end.y, end.z), height = end.x  - start.x;
        if (axis == 2) center = (end.x, end.y / 2, end.z), height = end.y - start.y;
        if (axis == 3) center = (end.x, end.y, end.z / 2), height = end.z - start.z;
        radius = r;
    ...
    bool intersect(const Vec3f& orig, const Vec3f& dir, float& tnear, uint32_t& index, Vec2f& uv) const
    {
        ...
        Vec3f rayInv = Vec3f(1 / dir.x, 1 / dir.y, 1 / dir.z);
        int sign[3];
        sign[0] = (rayInv.x < 0);
        sign[1] = (rayInv.y < 0);
        sign[2] = (rayInv.z < 0);

        float a, b, c, tcap0, tcap1, axCenter, po, di;
        Vec2f d_yz, o_yz, d_xz, o_xz, d_xy, o_xy;
        switch(axis)
        {
        case 1:
            d_yz = Vec2f(dir.y, dir.z);
            o_yz = Vec2f(orig.y, orig.z);

            axCenter = center.x;
            po = orig.x;
            di = dir.x;

            a = (dir.y * dir.y) + (dir.z * dir.z);
            b = 2 * (dir.y * (orig.y - center.y) + dir.z * (orig.z - center.z));
            c = (orig.y - center.y) * (orig.y - center.y) + (orig.z - center.z) * (orig.z - center.z) - (radius * radius);
            //tcap0 = (1 - orig.y) * rayInv.y;
            //tcap1 = (-1 - orig.z) * rayInv.z;

            //a = dotProduct2(d_yz, d_yz);

            //b = 2 * dotProduct2(d_yz, o_yz);

           // c =  dotProduct2(o_yz, o_yz) - (radius * radius);

            break;

        case 2:

            d_xz = Vec2f(dir.x, dir.z);
            o_xz = Vec2f(orig.x, orig.z);
            ...
            b = 2 * (dir.x * (orig.x - center.x) + dir.z * (orig.z - center.z));
            c = (orig.x - center.x) * (orig.x - center.x) + (orig.z - center.z) * (orig.z - center.z) - (radius * radius);
            //tcap0 = (1 - orig.z) * rayInv.z;
            //tcap1 = (-1 - orig.z) * rayInv.z;

            //a = dotProduct2(d_xz, d_xz);

            //b = 2 * dotProduct2(d_xz, o_xz);

            //c = dotProduct2(o_xz, o_xz) - (radius * radius);
            break;
          
        case 3:

            d_xy = Vec2f(dir.x, dir.y);
            o_xy = Vec2f(orig.x, orig.y);

            axCenter = center.z;
            po = orig.z;
            di = dir.z;

            a= (dir.x * dir.y) + (dir.z * dir.y);
            b= 2 * (dir.x * (orig.x - center.x) + dir.y * (orig.y - center.y));
            c= (orig.x - center.x) * (orig.x - center.x) + (orig.y - center.y) * (orig.y - center.y) - (radius * radius);
            //tcap0 = (1 - orig.x) * rayInv.x;
            //tcap1 = (-1 - orig.x) * rayInv.x;

            //a = dotProduct2(d_xy, d_xy);

           // b = 2 * dotProduct2(d_xy, o_xy);

            //c = dotProduct2(o_xy, o_xy) - (radius * radius);
        ...
        if ((r >= axCenter) && (r <= axCenter + height)) {
            float t0 = (1 - di) * 1/d;
            float t1 = (-1 - po) * 1/d;
            if (t0 > t1) std::swap(t0, t1);
    ...
    void getSurfaceProperties(const Vec3f& P, const Vec3f& I, const uint32_t& index, const Vec2f& uv, Vec3f& N, Vec2f& st) const
    {
        ...
        {
        case 1:
            N = normalize(Vec3f(P.y - center.y, 0, P.z - center.z));

            break;

        case 2:
            N = normalize(Vec3f(P.x - center.x, 0, P.z - center.z));
            break;

        case 3:
            N = normalize(Vec3f(P.x - center.x, 0, P.y - center.y));
...
{
public:
    AABBox(const Vec3f& c, const float& l) : center(c){
        
        bounds[0] = Vec3f((c.x - (l * 0.5)), (c.y - (l * 0.5)), (c.z - (l * 0.5)));
        bounds[1] = Vec3f((c.x + (l * 0.5)), (c.y + (l * 0.5)), (c.z + (l * 0.5)));
    ...
    bool intersect(const Vec3f& orig, const Vec3f& dir, float& tnear, uint32_t& index, Vec2f& uv) const
    {
        ...
        tmin = (bounds[sign[0]].x - orig.x) * rayInv.x;
        tmax = (bounds[1 - sign[0]].x - orig.x) * rayInv.x;
        tymin = (bounds[sign[1]].y - orig.y) * rayInv.y;
        tymax = (bounds[1 - sign[1]].y - orig.y) * rayInv.y;
        ...
        tzmin = (bounds[sign[2]].z - orig.z) * rayInv.z;
        tzmax = (bounds[1 - sign[2]].z - orig.z) * rayInv.z;
    ...
    void getSurfaceProperties(const Vec3f& P, const Vec3f& I, const uint32_t& index, const Vec2f& uv, Vec3f& N, Vec2f& st) const
    {
        Vec3f c = (bounds[0] + bounds[1]) * 0.5;
        Vec3f d = (bounds[0] - bounds[1]) * 0.5;
        N = normalize(((P.x / abs(d.x)), (P.y / abs(d.y)), (P.z / abs(d.z))));
...
int main(int argc, char **argv)
{
    ...
    AABBox *box1 = new AABBox(Vec3f(-1, 0, -12), 4);
    box1->materialType = DIFFUSE_AND_GLOSSY;
    box1->diffuseColor = Vec3f(0.6, 0.7, 0.8);

    objects.push_back(std::unique_ptr<AABBox>(box1));
    
    Cylinder *cylin = new Cylinder(Vec3f(0.5, -0.5, -8), Vec3f(0.5, 1, -8), 1);
    cylin->materialType = DIFFUSE_AND_GLOSSY;
    cylin->diffuseColor = Vec3f(0.6, 0.7, 0.8);

    objects.push_back(std::unique_ptr<Cylinder>(cylin));
